...
def get_git_url(title):
    ...
    ?repository fabio:hasURL ?url .
    {
        ?repository ^lpwc:hasOfficialRepository ?paper .
    }
    UNION
    {
        ?repository ^lpwc:hasRepository ?paper .
    }
    ?paper dcterms:title "?PAPER_TITLE".
    }
    """
    query_replace = query.replace("?PAPER_TITLE", f"{title}")
    ...
    try : 
        ...
        if results["results"]["bindings"] :
            lpwc_git = results["results"]["bindings"][0]["url"]["value"]
            return lpwc_git
        else :
            return ""
    ...
    try :
        ...
        if results["results"]["bindings"] :
            for result in results["results"]["bindings"] :
                ...
                task_refined = task.replace("https://linkedpaperswithcode.com/task/", "")
                task_list.append(task_refined)
            return task_list
        else :
            return []
    except Exception as e:
...
def parse(
    ...
    for fn in tqdm(fns, total=len(fns), unit='papers'):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir_path:
            ...
            title = paper_dict['metadata']['title']
            #paper_dict['git_url'] = get_git_url(title)
            #paper_dict['tasks'] = get_task_list(title)
            # parse XML
